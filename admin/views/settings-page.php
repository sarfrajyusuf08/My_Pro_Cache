...
<div class="wrap my-pro-cache-wrap mpc-dashboard-wrap">
    <header class="mpc-header">
        <div class="mpc-header__title">
            <h1><?php echo esc_html( $page['page_title'] ?? 'My Pro Cache' ); ?></h1>
        </div>
    </header>
    ...
    <?php if ( ! isset( $page['show_save'] ) || $page['show_save'] ) : ?>
        <!-- Settings form wrapper renders section fields -->
        <div class="mpc-settings-panel" role="region" aria-labelledby="mpc-settings-title">
            <h2 id="mpc-settings-title" class="screen-reader-text"><?php echo esc_html( $page['page_title'] ?? __( 'Settings', 'my-pro-cache' ) ); ?></h2>
            <form method="post" action="<?php echo esc_url( admin_url( 'options.php' ) ); ?>" class="mpc-settings-form">
                <?php
            ...
            <div class="mpc-settings-toolbar">
                <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="mpc-reset-form">
                    ...
                    <input type="hidden" name="_mpc_redirect" value="<?php echo esc_url( admin_url( 'admin.php?page=my-pro-cache-' . $slug ) ); ?>" />
                    <?php submit_button( __( 'Reset to Safe Defaults', 'my-pro-cache' ), 'secondary', 'submit', false ); ?>
                </form>
            </div>
        </div>
    <?php else : ?>
        <div class="mpc-settings-panel" role="region">
            <?php do_settings_sections( 'my-pro-cache-' . $slug ); ?>
        </div>
    <?php endif; ?>
</div>
